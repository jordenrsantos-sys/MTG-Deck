...
from api.engine.layers.commander_dependency_v2 import COMMANDER_DEPENDENCY_V2_VERSION
...
class PipelineCommanderDependencyV2Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_commander_dependency_v2_and_version_lock(self) -> None:
        ...
        commander_dep_payload = result.get("commander_dependency_v2")
        self.assertIsInstance(commander_dep_payload, dict)
        self.assertEqual(commander_dep_payload.get("version"), COMMANDER_DEPENDENCY_V2_VERSION)
        self.assertIn(commander_dep_payload.get("status"), ["OK", "WARN", "SKIP"])

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(
            pipeline_versions.get("commander_dependency_v2_version"),
