...
from api.engine.layers.substitution_engine_v1 import SUBSTITUTION_ENGINE_V1_VERSION
...
class PipelineSubstitutionEngineV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_substitution_engine_payload_and_panel_gate(self) -> None:
        ...
        substitution_payload = result.get("substitution_engine_v1")
        self.assertIsInstance(substitution_payload, dict)
        self.assertEqual(substitution_payload.get("version"), SUBSTITUTION_ENGINE_V1_VERSION)

        status = substitution_payload.get("status")
        ...
        self.assertIs(available_panels.get("has_substitution_engine_v1"), True)

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(pipeline_versions.get("substitution_rules_version"), "bucket_substitutions_v1")
        self.assertEqual(pipeline_versions.get("substitution_engine_version"), SUBSTITUTION_ENGINE_V1_VERSION)
