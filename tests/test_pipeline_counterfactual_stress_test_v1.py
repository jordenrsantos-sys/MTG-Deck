...
from api.engine.layers.counterfactual_stress_test_v1 import COUNTERFACTUAL_STRESS_TEST_V1_VERSION
...
class PipelineCounterfactualStressTestV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_counterfactual_payload_and_panel_gate(self) -> None:
        ...
        stress_payload = result.get("counterfactual_stress_test_v1")
        self.assertIsInstance(stress_payload, dict)
        self.assertEqual(stress_payload.get("version"), COUNTERFACTUAL_STRESS_TEST_V1_VERSION)

        status = stress_payload.get("status")
        self.assertIsInstance(status, str)
        self.assertIn(status, ["OK", "SKIP"])

        scenarios = stress_payload.get("scenarios") if isinstance(stress_payload.get("scenarios"), list) else []
        if status == "OK":
        ...
        self.assertIs(available_panels.get("has_counterfactual_stress_test_v1"), True)
