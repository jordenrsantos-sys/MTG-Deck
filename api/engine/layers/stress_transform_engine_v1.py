...
STRESS_TRANSFORM_ENGINE_V1_VERSION = "stress_transform_engine_v1"
_DECK_SIZE_N = 99
...
def _operator_sort_key(entry: Dict[str, Any]) -> tuple[str, str]:
    ...
    if op == "TARGETED_REMOVAL":
        return (op, f"{int(entry.get('count') or 0):010d}")
    if op == "BOARD_WIPE":
        return (
            op,
            f"{int(entry.get('by_turn') or 0):010d}:{_round6_half_up(float(entry.get('surviving_engine_fraction') or 0.0)):.6f}",
        )
    if op == "GRAVEYARD_HATE_WINDOW":
        turns = entry.get("turns") if isinstance(entry.get("turns"), list) else []
        turns_token = ",".join(str(int(turn)) for turn in turns if _is_nonnegative_int(turn))
        return (
            op,
            f"{turns_token}:{_round6_half_up(float(entry.get('graveyard_penalty') or 0.0)):.6f}",
        )
    if op == "STAX_TAX":
        return (
            op,
            f"{int(entry.get('by_turn') or 0):010d}:{_round6_half_up(float(entry.get('inflation_factor') or 0.0)):.6f}",
        )
    return (op, "")
...
def _base_payload(
    *,
    ...
    return {
        "version": STRESS_TRANSFORM_ENGINE_V1_VERSION,
        "status": status,
...
def _recompute_probabilities(
    *,
    ...
    for checkpoint in _CHECKPOINTS:
        ...
        try:
            probability_raw = hypergeom_p_ge_1(
                N=_DECK_SIZE_N,
                K_int=int(k_int),
                n=int(n_int),
            )
...
def run_stress_transform_engine_v1(
    *,
    ...
    math_status = _nonempty_str(probability_math_core_v1_payload.get("status"))
    if math_status != "OK":
        return _base_payload(
            status="SKIP",
            reason_code="PROBABILITY_MATH_CORE_UNAVAILABLE",
    ...
    stress_status = _nonempty_str(stress_model_definition_v1_payload.get("status"))
    if stress_status not in {"OK", "WARN"}:
        return _base_payload(
            status="SKIP",
            reason_code="STRESS_MODEL_DEFINITION_UNAVAILABLE",
            codes=[],
    ...
    bucket_rows = [row for row in buckets_raw if isinstance(row, dict)]
    bucket_rows_sorted = sorted(bucket_rows, key=lambda row: str((row or {}).get("bucket") or ""))
    ...
    if len(normalized_operators) == 0:
        codes.add("STRESS_TRANSFORM_OPERATOR_INVALID")

    if len(baseline_by_bucket) == 0 and len(bucket_rows) > 0:
        codes.add("STRESS_TRANSFORM_BUCKET_EFFECTIVE_K_INVALID")
        ...
        for bucket in sorted(current_state_by_bucket.keys()):
            ...
            if op_name == "TARGETED_REMOVAL":
                ...
                effective_k_after = _round6_half_up(
                    _clamp_k(
                        effective_k_before
                        * float(operator.get("surviving_engine_fraction") or 0.0)
                    )
                )
                k_int_after = int(floor(effective_k_after))
                recomputed = _recompute_probabilities(
                ...
                effective_k_after = _round6_half_up(
                    _clamp_k(
                        effective_k_before
                        * float(operator.get("graveyard_penalty") or 0.0)
                    )
                )
                k_int_after = int(floor(effective_k_after))
                recomputed = _recompute_probabilities(
            ...
            "probabilities_by_checkpoint": [
                {
                    ...
                    "p_ge_1": _round6_half_up(
                        float(current_state_by_bucket[bucket]["probabilities"].get(checkpoint) or 0.0)
                    ),
                }
                for checkpoint in _CHECKPOINTS
