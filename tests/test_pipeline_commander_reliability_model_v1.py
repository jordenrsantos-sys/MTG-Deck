...
from api.engine.layers.commander_reliability_model_v1 import COMMANDER_RELIABILITY_MODEL_V1_VERSION
...
class PipelineCommanderReliabilityModelV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_commander_reliability_payload_panel_gate_and_version_pin(self) -> None:
        ...
        reliability_payload = result.get("commander_reliability_model_v1")
        self.assertIsInstance(reliability_payload, dict)
        self.assertEqual(reliability_payload.get("version"), COMMANDER_RELIABILITY_MODEL_V1_VERSION)

        status = reliability_payload.get("status")
        ...
        self.assertIs(available_panels.get("has_commander_reliability_model_v1"), True)

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(
            pipeline_versions.get("commander_reliability_model_version"),
