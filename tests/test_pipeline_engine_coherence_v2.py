...
from api.engine.layers.engine_coherence_v2 import ENGINE_COHERENCE_V2_VERSION
...
class PipelineEngineCoherenceV2Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_engine_coherence_v2_and_version_lock(self) -> None:
        ...
        coherence_payload = result.get("engine_coherence_v2")
        self.assertIsInstance(coherence_payload, dict)
        self.assertEqual(coherence_payload.get("version"), ENGINE_COHERENCE_V2_VERSION)
        self.assertIn(coherence_payload.get("status"), ["OK", "WARN", "SKIP"])

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(
            pipeline_versions.get("engine_coherence_v2_version"),
