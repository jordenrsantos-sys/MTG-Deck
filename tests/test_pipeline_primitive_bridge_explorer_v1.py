...
from api.engine.layers.primitive_bridge_explorer_v1 import PRIMITIVE_BRIDGE_EXPLORER_VERSION
...
class PipelinePrimitiveBridgeExplorerV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_bridge_payload_panel_gate_and_version_pin(self) -> None:
            ...
            patch(
                ...
                return_value={
                    "oracle_missing_commander": {
                        "primitive_ids": ["COMMANDER_SUPPORT", "TOKEN_PRODUCTION"],
                        "ruleset_version": "ruleset_v_test",
                        "evidence": {"matches": []},
                    },
                    "oracle_missing_card_a": {
                        "primitive_ids": ["TOKEN_PRODUCTION", "SAC_OUTLET"],
                        "ruleset_version": "ruleset_v_test",
                        "evidence": {"matches": []},
                    },
                    "oracle_missing_card_b": {
                        "primitive_ids": ["SAC_OUTLET", "RECURSION_TO_HAND"],
                        "ruleset_version": "ruleset_v_test",
                        "evidence": {"matches": []},
                    },
                },
        ...
        bridge_payload = result.get("primitive_bridge_explorer_v1")
        self.assertIsInstance(bridge_payload, dict)
        self.assertEqual(bridge_payload.get("version"), PRIMITIVE_BRIDGE_EXPLORER_VERSION)

        bridge_clusters = bridge_payload.get("bridge_clusters_v1") if isinstance(bridge_payload.get("bridge_clusters_v1"), list) else []
        ...
        self.assertIs(available_panels.get("primitive_bridge_explorer_v1"), True)

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(
            pipeline_versions.get("primitive_bridge_explorer_version"),
