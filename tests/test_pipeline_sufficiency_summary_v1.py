...
from api.engine.layers.sufficiency_summary_v1 import SUFFICIENCY_SUMMARY_V1_VERSION
...
class PipelineSufficiencySummaryV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_sufficiency_summary_payload_panel_gate_and_version_pins(self) -> None:
        ...
        sufficiency_payload = result.get("sufficiency_summary_v1")
        self.assertIsInstance(sufficiency_payload, dict)
        self.assertEqual(sufficiency_payload.get("version"), SUFFICIENCY_SUMMARY_V1_VERSION)

        status = sufficiency_payload.get("status")
        self.assertIsInstance(status, str)
        self.assertIn(status, ["PASS", "WARN", "FAIL", "SKIP"])

        available_panels = result.get("available_panels_v1") if isinstance(result.get("available_panels_v1"), dict) else {}
        self.assertIs(available_panels.get("has_sufficiency_summary_v1"), True)

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(
            pipeline_versions.get("sufficiency_summary_version"),
            SUFFICIENCY_SUMMARY_V1_VERSION,
        )
        self.assertEqual(
            pipeline_versions.get("profile_thresholds_version"),
