...
from api.engine.layers.engine_coherence_v1 import ENGINE_COHERENCE_V1_VERSION
...
class PipelineEngineCoherenceV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_engine_coherence_payload_and_panel_gate(self) -> None:
        ...
        coherence_payload = result.get("engine_coherence_v1")
        self.assertIsInstance(coherence_payload, dict)
        self.assertEqual(coherence_payload.get("version"), ENGINE_COHERENCE_V1_VERSION)

        status = coherence_payload.get("status")
        ...
        self.assertIs(available_panels.get("has_engine_coherence_v1"), True)

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(pipeline_versions.get("engine_coherence_version"), ENGINE_COHERENCE_V1_VERSION)
