...
from api.engine.layers.engine_requirement_detection_v1 import ENGINE_REQUIREMENT_DETECTION_V1_VERSION
...
class PipelineEngineRequirementDetectionV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_engine_requirement_detection_payload_and_panel_gate(self) -> None:
        ...
        self.assertEqual(engine_req_payload.get("version"), ENGINE_REQUIREMENT_DETECTION_V1_VERSION)

        status = engine_req_payload.get("status")
        ...
        self.assertIs(available_panels.get("has_engine_requirement_detection_v1"), True)

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(pipeline_versions.get("dependency_signatures_version"), "dependency_signatures_v1")
