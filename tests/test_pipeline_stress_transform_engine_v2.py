...
from api.engine.layers.stress_transform_engine_v2 import STRESS_TRANSFORM_ENGINE_V2_VERSION
...
class PipelineStressTransformEngineV2Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_stress_transform_v2_and_policy_payload(self) -> None:
        ...
        transform_payload = result.get("stress_transform_engine_v2")
        self.assertIsInstance(transform_payload, dict)
        self.assertEqual(transform_payload.get("version"), STRESS_TRANSFORM_ENGINE_V2_VERSION)
        self.assertIn(transform_payload.get("status"), ["OK", "WARN", "SKIP", "ERROR"])

        policy_payload = result.get("stress_operator_policy_v1")
        self.assertIsInstance(policy_payload, dict)
        self.assertEqual(policy_payload.get("version"), "stress_operator_policy_v1")

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(
            pipeline_versions.get("stress_transform_v2_version"),
            STRESS_TRANSFORM_ENGINE_V2_VERSION,
        )
        self.assertEqual(
            pipeline_versions.get("stress_operator_policy_version"),
