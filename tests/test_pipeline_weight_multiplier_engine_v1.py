...
from api.engine.layers.weight_multiplier_engine_v1 import WEIGHT_MULTIPLIER_ENGINE_V1_VERSION
...
class PipelineWeightMultiplierEngineV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_weight_multiplier_payload_and_panel_gate(self) -> None:
        ...
        weight_multiplier_payload = result.get("weight_multiplier_engine_v1")
        self.assertIsInstance(weight_multiplier_payload, dict)
        self.assertEqual(weight_multiplier_payload.get("version"), WEIGHT_MULTIPLIER_ENGINE_V1_VERSION)

        status = weight_multiplier_payload.get("status")
        self.assertIsInstance(status, str)
        self.assertIn(status, ["OK", "WARN", "SKIP"])

        available_panels = result.get("available_panels_v1") if isinstance(result.get("available_panels_v1"), dict) else {}
        self.assertIs(available_panels.get("has_weight_multiplier_engine_v1"), True)

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(pipeline_versions.get("weight_rules_version"), "weight_rules_v1")
        self.assertEqual(pipeline_versions.get("weight_multiplier_engine_version"), WEIGHT_MULTIPLIER_ENGINE_V1_VERSION)
