...
from api.engine.layers.redundancy_index_v1 import REDUNDANCY_INDEX_V1_VERSION
...
class PipelineRedundancyIndexV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_redundancy_payload_and_panel_gate(self) -> None:
        ...
        redundancy_payload = result.get("redundancy_index_v1")
        self.assertIsInstance(redundancy_payload, dict)
        self.assertEqual(redundancy_payload.get("version"), REDUNDANCY_INDEX_V1_VERSION)

        status = redundancy_payload.get("status")
        ...
        self.assertIs(available_panels.get("has_redundancy_index_v1"), True)
