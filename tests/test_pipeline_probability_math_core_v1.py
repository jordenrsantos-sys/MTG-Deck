...
from api.engine.layers.probability_math_core_v1 import PROBABILITY_MATH_CORE_V1_VERSION
...
class PipelineProbabilityMathCoreV1Tests(unittest.TestCase):
    ...
    def test_pipeline_reports_probability_math_core_payload_and_panel_gate(self) -> None:
        ...
        probability_payload = result.get("probability_math_core_v1")
        self.assertIsInstance(probability_payload, dict)
        self.assertEqual(probability_payload.get("version"), PROBABILITY_MATH_CORE_V1_VERSION)

        status = probability_payload.get("status")
        ...
        self.assertIs(available_panels.get("has_probability_math_core_v1"), True)

        pipeline_versions = result.get("pipeline_versions") if isinstance(result.get("pipeline_versions"), dict) else {}
        self.assertEqual(pipeline_versions.get("probability_model_version"), PROBABILITY_MATH_CORE_V1_VERSION)
