"""Pillar D iter 6 — 5-case validation sweep (mega-task v5 Phase 13).

Identical 5 test cases to iter 2/3/4/5. Computes the 12 iter-6 success
criteria — same five baseline metrics (iter1 / creativity / novel /
cost / wallclock) plus the new iter-6 targets:

  6. voyage_semantic_avg >= 3                 (Phase 6 fix: venv-recovered Voyage retrieval)
  7. intent_drift_mean < 0.5                  (Phase 7 fix: archetype-aware drift)
  8. atraxa_c2_1_latency > 0                  (Phase 8 fix: dynamic budget overhead)
  9. pillar_e_v0_3_curve_check on 5/5         (Phase 9 presence)
 10. pillar_e_v0_4_interaction within target on 4/5 (Phase 10)
 11. graduated_playtest_report present on 5/5 (Phase 12)
 12. ui-equivalent build path completes on 5/5 (chrome-devtools-mcp
     substitute: each case's build returns OK + 100-card deck within
     the 240s Phase-4 timeout ceiling)

Writes report to
`api/engine/data/agent/pillar_d_iteration_6_validation_report.md`.

Halt condition (per kickoff hard-halt #5): if >= 3 of 12 criteria fail,
the report STILL gets written but the script returns exit code 1 so a
caller can halt for user direction before Phase 14.
"""
...
REPORT_PATH = REPO_ROOT / "api" / "engine" / "data" / "agent" / "pillar_d_iteration_6_validation_report.md"
...
def main(argv=None) -> int:
    ...
    print(flush=True)
    print(f"=== iter 6 success criteria (passed {passed}/12) ===", flush=True)
