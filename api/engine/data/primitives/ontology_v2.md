# Pillar C — Primitive Ontology v2 (iter 7 mega-task v6 expansion)

Status: SHIPPED in mega-task v6 Phase 3. Extends v1 (64 v0 + 17
rules_modifiers = 81 tags) with an 8th dimension
`counters_and_proliferate` (12 tags) so the classifier can identify
counters-matter archetypes (Atraxa proliferate, Pir+Toothy, Hardened
Scales modular, etc.) on REAL primitive signal rather than the v5
Phase 7 anthem-effect proxy that caused tribal-dilution drift on
Atraxa.

**Total tag count: 93 across 8 dimensions (64 v0 + 17 rules_modifiers + 12 counters_and_proliferate).**

This document defines the primitive-tag vocabulary the agent uses
to reason about cards semantically. Each tag spans one of 7 dimensions
and has a regex/text-pattern extraction rule precise enough that a
batch extractor can populate the `cards.primitives_v1_json` column
without ambiguity for the majority of cards. Cards where regex
extraction returns <2 tags are routed to the LLM-extractor supplement
(`primitive_extractor_llm_v1.py`) for additional tagging on a per-card
basis (~$0.001 per call; restricted to ambiguous cards).

**v1 baseline: 81 across 7 dimensions (64 v0 + 17 rules_modifiers).**
...
## Dimension 7: Rules modifiers (17 tags) — iter 5 expansion

How a card's effect interacts with the rules layer. These tags
distinguish between cards whose oracle text is otherwise nearly
identical at the effect level. Example: "Whenever a land enters, you
**may** put a token" (optional-trigger) vs "you put a token"
(mandatory-trigger) — same effect-level primitive (landfall +
token-producer) but opposite gameplay.

### mandatory-trigger
- dimension: rules_modifiers
- definition: A triggered ability with no "may" — always resolves when the condition is met.
- extraction_rule:
  - `^(whenever|when|at the beginning of)[^.]{0,200}(?<!may )(?<!may  )(?<!may   )(?:draw|destroy|create|deal|put|exile|return|gain|lose|sacrifice|reveal|search|tap)`
- examples: ["Edgar Markov", "Avenger of Zendikar", "Tireless Tracker"]
- combos_with: [etb-trigger, attack-trigger, death-trigger]

### optional-trigger
- dimension: rules_modifiers
- definition: A triggered ability worded "you may" — controller chooses whether to resolve.
- extraction_rule:
  - `(whenever|when|at the beginning of)[^.]{0,80}you may`
- examples: ["Lotus Cobra", "Sun Titan", "Murderous Redcap"]
- combos_with: [etb-trigger, recursion-graveyard]

### activated-ability-mana-cost
- dimension: rules_modifiers
- definition: Activated ability whose cost contains mana symbols.
- extraction_rule:
  - `\{[0-9XWUBRGCSP/]+\}[^:]{0,20}:`
- examples: ["Walking Ballista", "Birthing Pod", "Sword of Truth and Justice"]
- combos_with: [infinite-mana-source, x-spell-payoff]

### activated-ability-tap-cost
- dimension: rules_modifiers
- definition: Activated ability with `{T}` as part of the cost.
- extraction_rule:
  - `\btap\b.{0,40}:`
- examples: ["Sol Ring", "Krenko, Mob Boss", "Voltaic Key"]
- combos_with: [infinite-untap-source, untap-extra]

### sacrifice-as-cost
- dimension: rules_modifiers
- definition: Cost includes a sacrifice action (vs sacrifice as effect).
- extraction_rule:
  - `sacrifice (a|an|another|this)[^:]{0,30}:`
- examples: ["Viscera Seer", "Phyrexian Altar", "Birthing Pod"]
- combos_with: [sac-outlet, death-trigger]

### combat-damage-only-trigger
- dimension: rules_modifiers
- definition: Trigger fires only on COMBAT damage, not any damage.
- extraction_rule:
  - `(whenever|when)[^.]{0,80}deal[s]? combat damage`
- examples: ["Edric, Spymaster of Trest", "Tymna the Weaver", "Yuriko, the Tiger's Shadow"]
- combos_with: [attack-trigger, evasion-grant]

### any-damage-trigger
- dimension: rules_modifiers
- definition: Trigger fires on ANY damage (combat or non-combat).
- extraction_rule:
  - `(whenever|when)[^.]{0,40}is dealt damage|deal[s]? damage to (any|a)`
- examples: ["Stuffy Doll", "Boros Reckoner", "Bloodthirsty Conqueror"]
- combos_with: [infinite-damage-source]

### targeted-effect
- dimension: rules_modifiers
- definition: Effect requires the controller to choose a specific target on cast/activation.
- extraction_rule:
  - `target (creature|player|opponent|planeswalker|artifact|enchantment|land|permanent|spell)`
- examples: ["Swords to Plowshares", "Counterspell", "Demonic Tutor"]
- combos_with: [counterspell-hard, removal-creature]

### any-target
- dimension: rules_modifiers
- definition: "Any target" — can hit creatures, planeswalkers, players, battles flexibly.
- extraction_rule:
  - `any target`
- examples: ["Lightning Bolt", "Walking Ballista", "Heliod, Sun-Crowned"]
- combos_with: [infinite-damage-source, x-spell-payoff]

### untargeted-effect
- dimension: rules_modifiers
- definition: Effect resolves without picking a target (mass effects, "each opponent", etc.).
- extraction_rule:
  - `each (player|opponent|creature|permanent)|all (creatures|permanents)`
- examples: ["Wrath of God", "Damnation", "Cyclonic Rift"]
- combos_with: [removal-mass-creatures, removal-mass-board]

### controller-only-effect
- dimension: rules_modifiers
- definition: Effect applies only to permanents YOU control (vs all).
- extraction_rule:
  - `(creatures|permanents|lands|artifacts|enchantments) you control`
- examples: ["Anointed Procession", "Heroic Intervention", "Lightning Greaves"]
- combos_with: [creature-protection, anthem-effect]

### opponent-only-effect
- dimension: rules_modifiers
- definition: Effect applies only to opponents' permanents/zones.
- extraction_rule:
  - `(each opponent|target opponent|opponents control|an opponent)`
- examples: ["Cyclonic Rift", "Mass Diminish", "Sanctum Seeker"]
- combos_with: [removal-mass-creatures, attack-trigger]

### replacement-effect
- dimension: rules_modifiers
- definition: Modifies how an event would happen instead of triggering after — "if/instead", "would ... instead".
- extraction_rule:
  - `if .{0,80}would .{0,40}instead`
  - `\binstead\b`
- examples: ["Doubling Season", "Anointed Procession", "Hardened Scales"]
- combos_with: [doubler-effect, token-producer]

### static-ability
- dimension: rules_modifiers
- definition: Continuous static effect (anthem, cost-reduction, granted keyword) — no trigger required.
- extraction_rule:
  - `(creatures|spells|lands) you control (have|get)`
  - `(have|gains?|has) (haste|flying|hexproof|indestructible|trample|vigilance)`
- examples: ["Glorious Anthem", "Lightning Greaves", "Vedalken Orrery"]
- combos_with: [anthem-effect, haste-grant, evasion-grant]

### cast-trigger
- dimension: rules_modifiers
- definition: Triggers when YOU cast a spell (vs when a permanent enters).
- extraction_rule:
  - `whenever you cast`
- examples: ["Aetherflux Reservoir", "Niv-Mizzet, Parun", "Storm-Kiln Artist"]
- combos_with: [storm-payoff, cantrip]

### enter-the-battlefield-self
- dimension: rules_modifiers
- definition: ETB trigger fires when THIS card enters, not arbitrary creatures.
- extraction_rule:
  - `when [a-z][a-z\-' ]{0,30} enters`
- examples: ["Mulldrifter", "Solemn Simulacrum", "Snapcaster Mage"]
- combos_with: [etb-trigger, flicker-effect, alternative-cost]

### enter-the-battlefield-any
- dimension: rules_modifiers
- definition: ETB trigger fires when ANY (or another) creature enters.
- extraction_rule:
  - `whenever a (creature|permanent|nontoken) (?:.{0,40})?enters`
  - `whenever another (creature|permanent)`
- examples: ["Panharmonicon", "Soul Warden", "Impact Tremors"]
- combos_with: [etb-trigger, flicker-effect, token-producer]

---

## Dimension 8: Counters and proliferate (12 tags) — iter 7 expansion

How a card interacts with counter mechanics. Replaces the v5 Phase 7
`anthem-effect` proxy that the iter 6 sweep flagged as causing tribal-
dilution drift on Atraxa (mean drift 0.882 vs target ≤0.7). Cards
where these tags fire belong in counters_matter archetypes that the
theme classifier can now identify directly rather than via the noisier
anthem proxy.

### proliferate-trigger
- dimension: counters_and_proliferate
- definition: Proliferates or triggers proliferation (each kind of counter already on a permanent or player gets one more of its kind).
- extraction_rule:
  - `\bproliferate\b`
- examples: ["Inexorable Tide", "Contagion Engine", "Karn's Bastion"]
- combos_with: [plus1plus1-counter-payoff, charge-counter-payoff, loyalty-counter-payoff]

### plus1plus1-counter-distributor
- dimension: counters_and_proliferate
- definition: Places +1/+1 counters on one or more creatures as a primary effect.
- extraction_rule:
  - `put[s]?[^.]{0,60}\+1/\+1 counter[s]?[^.]{0,60}on`
  - `\bmodular\b`
  - `bolster `
  - `evolve\b`
  - `with [a-z\- ]{0,30} \+1/\+1 counters? on it`
- examples: ["Hardened Scales", "Cathars' Crusade", "Conclave Mentor"]
- combos_with: [plus1plus1-counter-doubler, proliferate-trigger]

### plus1plus1-counter-doubler
- dimension: counters_and_proliferate
- definition: Causes additional / doubled +1/+1 counters to be placed (Doubling Season / Hardened Scales / Branching Evolution pattern).
- extraction_rule:
  - `if .{0,80}would be put on .{0,80}\+1/\+1 counter`
  - `(twice|double) the number of .{0,40}\+1/\+1 counter`
  - `one more.{0,40}\+1/\+1 counter`
  - `that many plus.{0,30}\+1/\+1 counter`
  - `(twice|double) that many .{0,30}counter`
- examples: ["Doubling Season", "Hardened Scales", "Branching Evolution"]
- combos_with: [plus1plus1-counter-distributor, proliferate-trigger]

### plus1plus1-counter-payoff
- dimension: counters_and_proliferate
- definition: Card benefits when +1/+1 counters are present on it or on creatures you control (Pir/Toothy/Krark-Clan Ironworks-of-creatures pattern).
- extraction_rule:
  - `for each \+1/\+1 counter`
  - `with a \+1/\+1 counter on it`
  - `as long as .{0,60}\+1/\+1 counter`
- examples: ["Pir, Imaginative Rascal", "Forgotten Ancient", "Animar, Soul of Elements"]
- combos_with: [plus1plus1-counter-distributor, proliferate-trigger]

### minus1minus1-counter-distributor
- dimension: counters_and_proliferate
- definition: Places -1/-1 counters on creatures (Wither/Infect/Persist pattern).
- extraction_rule:
  - `put[s]?[^.]{0,60}\-1/\-1 counter[s]?[^.]{0,60}on`
  - `\binfect\b`
  - `\bwither\b`
- examples: ["Black Sun's Zenith", "Phyrexian Hulk", "Hapatra, Vizier of Poisons"]
- combos_with: [proliferate-trigger, persist-creature]

### charge-counter-payoff
- dimension: counters_and_proliferate
- definition: Card scales or activates based on charge counters present.
- extraction_rule:
  - `charge counter`
  - `\{q\}.{0,20}charge`
- examples: ["Coretapper", "Power Conduit", "Energy Chamber"]
- combos_with: [proliferate-trigger]

### loyalty-counter-payoff
- dimension: counters_and_proliferate
- definition: Card adds loyalty counters to planeswalkers or scales off loyalty.
- extraction_rule:
  - `loyalty counter`
  - `loyalty abilit`
- examples: ["Carth the Lion", "Doubling Season", "The Chain Veil"]
- combos_with: [proliferate-trigger]

### energy-counter-producer
- dimension: counters_and_proliferate
- definition: Produces energy counters {E} for the controller.
- extraction_rule:
  - `get .{0,20}\{e\}`
  - `get .{0,40}energy counter`
  - `gain.{0,30}energy counter`
- examples: ["Aetherworks Marvel", "Whirler Virtuoso", "Attune with Aether"]
- combos_with: [energy-counter-payoff]

### energy-counter-payoff
- dimension: counters_and_proliferate
- definition: Spends or scales off energy counters as a payoff.
- extraction_rule:
  - `pay \{e\}`
  - `pay .{0,20}energy counter`
  - `spend .{0,30}energy`
- examples: ["Aetherworks Marvel", "Whirler Virtuoso", "Longtusk Cub"]
- combos_with: [energy-counter-producer, x-spell-payoff]

### keyword-counter-producer
- dimension: counters_and_proliferate
- definition: Distributes keyword counters (flying, haste, vigilance, lifelink, etc.) onto creatures.
- extraction_rule:
  - `(flying|haste|first strike|double strike|vigilance|lifelink|trample|menace|reach|deathtouch|hexproof|indestructible) counter`
- examples: ["Heroic Reinforcements", "Setessan Champion", "Akroma, Vision of Ixidor"]
- combos_with: [anthem-effect, evasion-grant, proliferate-trigger]

### counter-removal-or-relocation
- dimension: counters_and_proliferate
- definition: Removes counters from one permanent or moves them between permanents (Vorel-of-the-Hull-Clade / Power Conduit pattern).
- extraction_rule:
  - `remove .{0,30}counter`
  - `move .{0,30}counter`
  - `double the number of .{0,40}counter`
  - `another counter of that kind`
- examples: ["Vorel of the Hull Clade", "Power Conduit", "Fertilid"]
- combos_with: [plus1plus1-counter-payoff, proliferate-trigger]

### counter-trigger-scaling
- dimension: counters_and_proliferate
- definition: An ability scales with or triggers per counter present (Inexorable Tide / Mer-Ek Nightblade pattern).
- extraction_rule:
  - `whenever .{0,40}counter is put on`
  - `for each counter`
  - `as long as .{0,40}counter on`
- examples: ["Inexorable Tide", "Forgotten Ancient", "Reyhan, Last of the Abzan"]
- combos_with: [proliferate-trigger, plus1plus1-counter-payoff]
